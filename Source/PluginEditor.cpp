...
static void layoutGraphicsPopupContent (juce::AlertWindow& aw)
{
	...
	const int fxLabelW = (fxLabel != nullptr) ? juce ::jmax (90, stringWidth (fxLabel->getFont(), fxLabel->getText().toUpperCase()) + 2)
	                   : 96;
...
const CABTRAudioProcessorEditor::LoaderParamIds CABTRAudioProcessorEditor::kLoaderParams[3] =
{
	{ // A
		...
		CABTRAudioProcessor::kParamVariationA,
	...
	{ // B
		...
		CABTRAudioProcessor::kParamVariationB,
	...
	{ // C
		...
		CABTRAudioProcessor::kParamVariationC,
...
CABTRAudioProcessorEditor::LoaderRefs CABTRAudioProcessorEditor::getLoaderRefs (int i)
{
	...
	case 1: return { enableButtonB, browseButtonB, fileDisplayB,
	                 ...
	                 invButtonB, normButtonB, rvsButtonB, expButtonB, chaosButtonB, chaosFilterButtonB, chaosDisplayB, chaosFilterDisplayB, expDisplayB,
	...
	case 2: return { enableButtonC, browseButtonC, fileDisplayC,
	                 ...
	                 invButtonC, normButtonC, rvsButtonC, expButtonC, chaosButtonC, chaosFilterButtonC, chaosDisplayC, chaosFilterDisplayC, expDisplayC,
	...
	default: return { enableButtonA, browseButtonA, fileDisplayA,
	                  ...
	                  invButtonA, normButtonA, rvsButtonA, expButtonA, chaosButtonA, chaosFilterButtonA, chaosDisplayA, chaosFilterDisplayA, expDisplayA,
                                                          ...
                                                          LoaderRefs ui, AttachRefs a)
{
	...
	a.variationAtt = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment> (params, ids.variation, ui.variation);
...
void CABTRAudioProcessorEditor::resized()
{
	...
	{
		const auto exportArea = getExportIconArea();
		const int alignX = exportArea.getRight() + 8;
		const int alignY = exportArea.getY();
		const int alignW = 80;
		const int alignH = exportArea.getHeight();
...
void CABTRAudioProcessorEditor::layoutIRSection (juce::Rectangle<int> area, int loaderIndex)
{
	...
	auto& variation = pick (variationSliderA, variationSliderB, variationSliderC);
	...
	else
	{
		...
		expDisp.setBounds (labelAreaForToggle (exp));  expDisp.setVisible (true);
...
void CABTRAudioProcessorEditor::updateLoaderEnabledState (int loaderIndex)
{
	...
	juce::Component* components[] = {
		...
		&r.inv, &r.norm, &r.rvs, &r.exp, &r.expDisp, &r.chaos, &r.chaosFilter, &r.chaosDisp, &r.chaosFilterDisp,
...
void CABTRAudioProcessorEditor::hideLoaderSection (int loaderIndex)
{
	...
	juce::Component* components[] = {
		...
		&refs.chaos, &refs.chaosFilter, &refs.chaosDisp, &refs.chaosFilterDisp,
...
void CABTRAudioProcessorEditor::openNumericEntryPopupForSlider (juce::Slider& s)
{
	...
	if (auto* te = aw->getTextEditor ("val"))
	{
		...
		{
			minVal = CABTRAudioProcessor::kLimThresholdMin;
			maxVal = CABTRAudioProcessor::kLimThresholdMax;
	...
	aw->enterModalState (true,
		juce::ModalCallbackFunction::create ([safeThis, sliderPtr, aw] (int result) mutable
		{
			...
			const bool needsPercentConvert = (st == BarSlider::Type::Size || st == BarSlider::Type::Pan ||
			                                  ...
			                                  st == BarSlider::Type::Reso  || st == BarSlider::Type::Variation ||
...
void CABTRAudioProcessorEditor::openExpPrompt (int loaderIndex)
{
	...
	const int currentScHpSlope = juce::jlimit (CABTRAudioProcessor::kFilterSlopeMin,
	...
	const int currentScLpSlope = juce::jlimit (CABTRAudioProcessor::kFilterSlopeMin,
	                                           CABTRAudioProcessor::kFilterSlopeMax,
	...
	{
		const float clamped = juce::jlimit (CABTRAudioProcessor::kExpScFreqMin,
		                                    CABTRAudioProcessor::kExpScFreqMax,
		                                    hz);
		if (clamped >= 10000.0f)
			return juce::String (juce::roundToInt (clamped / 1000.0f));
		if (clamped >= 1000.0f)
	...
	auto* scHpBar = new PromptBar (scheme, scHpNorm, freqToNorm (CABTRAudioProcessor::kExpScHpDefault));
	auto* scLpBar = new PromptBar (scheme, scLpNorm, freqToNorm (CABTRAudioProcessor::kExpScLpDefault));
	...
	{
		if (te == nullptr || unitLabel == nullptr)
			return juce::jlimit (CABTRAudioProcessor::kExpScFreqMin,
			                     CABTRAudioProcessor::kExpScFreqMax,
			                     fallbackHz);
	...
	*layoutBodyFn = layoutBody;
	                         ...
	                         aw, scHpBar, scLpBar] (bool isHp) mutable
	{
		...
		*syncing = true;
		...
		const float fallback = isHp ? CABTRAudioProcessor::kExpScHpDefault : CABTRAudioProcessor::kExpScLpDefault;
		const float otherFallback = isHp ? CABTRAudioProcessor::kExpScLpDefault : CABTRAudioProcessor::kExpScHpDefault;
			 ...
			 loaderIndex] (int result) mutable
		{
			...
			else
			{
				...
				const int newScHpSlope = juce::jlimit (CABTRAudioProcessor::kFilterSlopeMin,
				                                       CABTRAudioProcessor::kFilterSlopeMax,
				                                       *scHpSlopeState);
				const int newScLpSlope = juce::jlimit (CABTRAudioProcessor::kFilterSlopeMin,
				                                       CABTRAudioProcessor::kFilterSlopeMax,
				                                       *scLpSlopeState);
...
void CABTRAudioProcessorEditor::openExportPrompt()
{
	...
	aw->enterModalState (true,
			...
			[safeThis, aw, formPanel, rateCombo, formatCombo, lengthLabel, trimToggle, normalizeToggle, mptToggle] (int result) mutable
		{
			...
			const float maxLen = (lengthLabel != nullptr) ? juce ::jlimit (0.01f, 10.0f, lengthLabel->getText().trimCharactersAtEnd ("s").getFloatValue()) : 10.0f;
